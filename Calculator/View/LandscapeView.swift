...
struct LandscapeView: View {
    ...
    @State private var btnData: [[BtnType]] = landscapeBtn
                ...
                VStack(spacing: 0) {
                    ...
                    HStack(alignment:. bottom, spacing: 0) {
                        ...
                        VStack(spacing: 0) {
                            ...
                            .frame(height: calcVM.btnSize * 0.5)  //  ScrollView 내부 Text와 크기 같을 것
                            .scaleEffect(x: -1, y: 1)
                            ...
                            HStack {
                                ForEach(col, id: \.self) { button in
                                    Button(action: {
                                        ...
                                        ButtonLabelView(button: button.BtnDisplay)
                                            .frame(width: geometry.size.width / CGFloat(col.count) - 8,   // 8은 .padding()의 기본값
                                                   height: geometry.size.height * 0.75 / CGFloat(col.count) - 8,
                                                   alignment: .center
                                            )
                                            .background(calcVM.modeOn && button == BtnType.emoji ? Color.clear : button.backgroundColor)
                                            .cornerRadius(geometry.size.width / CGFloat(col.count) - 8)
                                    ...
                                    .onAppear {
                                        calcVM.btnSize = geometry.size.height / CGFloat(col.count) - 16
                                    }
                                }
                            }
                        }
